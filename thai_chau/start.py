...
"""After the first block are the blocks to calculate features using
    the functions in the first block"""
"""The comments on the code are sometimes writen in function print"""
...
data_folder = '/mva_link_prediction/data/' #Your data folder is here
code_folder = '/mva_link_prediction/code/' #Your code folder is here
...
def create_word_count(node_info):
    ...
    word_mat = lil_matrix((len(node_info), len(unique_words)), dtype=np.double)
...
print ('Computing authors features (a few long...)')
...
"""Common citation features"""
features += ['commonlink_temp26']
features += ['commonlink_temp2']

"""Date difference features"""
features += ['is_same_journal', 'time_difference']

"""Common author features"""
...
if dev_mode:
    ...
    X_train = train.iloc[idx_perm[:n_used_data]][features]
    y_train = train.iloc[idx_perm[:n_used_data]][to_predict]

    """Specify the list of classifiers here"""
    ...
    for (name, predictor) in zip(names, predictors):
        cv_scores = cross_validation.cross_val_score(predictor, X_train, y_train, cv=4, n_jobs=-1)
        print ('CV accuracy on {name} is {acc}'.format(name=name,acc=np.mean(cv_scores)))
else:
    X_train = train[features]
    y_train = train[to_predict]
    X_test = test[features]
    # Choose the best classifier here
    predictor = RandomForestClassifier(n_estimators=10, n_jobs=4, max_depth=9)
    predictor.fit(X_train, y_train)
    # Make predictions
    y_test_pred = predictor.predict(X_test)
    write_submit(y_test_pred, name_suffix = '_10')